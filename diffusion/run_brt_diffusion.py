...
from diffusion.utils import plot_value_function_batch
...
def train(args):
    ...
    model = UNet_conditional_FiLM(c_in=channels, c_out=channels, grid_size=grid_size, device=device).to(device)
    ...
    diffusion = Diffusion(img_size=grid_size, 
    ...
    for epoch in range(args.epochs):
        ...
        if epoch % args.save_every == 0:
            ...
            plot_value_function_batch(sampled_images, time=None, 
                                      save_path=os.path.join("results", args.run_name, f"{epoch}.png"))
            plot_value_function_batch(ema_sampled_images, time=None, 
...
def get_args():
    ...
    parser.add_argument("--run_name", type=str, default="PETE_BRT_diffusion")
    ...
    parser.add_argument("--noise_steps", type=int, default=100)
    parser.add_argument("--beta_start", type=float, default=1e-4)
    parser.add_argument("--beta_end", type=float, default=0.02)
    parser.add_argument("--num_samples_plot", type=int, default=10)
