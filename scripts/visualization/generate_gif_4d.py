"""
Generate a GIF showing the denoising process for a sample from the 4D dataset.
"""
...
def generate_denoising_gif(model, dataset, sample_idx, num_frames=50, save_dir="plots/gifs"):
    ...
    # Get all denoising sample values
    all_c_values = [c_gt]
    for sample in selected_samples:
        points = sample[0].cpu().numpy()
        points = dataset.denormalize_points(points)
        c_values = points[:, 3]
        all_c_values.append(c_values)
    
    # Calculate global min/max and make symmetric around zero for better pos/neg distinction
    all_c_concat = np.concatenate(all_c_values)
    c_min_raw, c_max_raw = np.min(all_c_concat), np.max(all_c_concat)
    c_abs_max = max(abs(c_min_raw), abs(c_max_raw))
    c_min, c_max = -c_abs_max, c_abs_max  # Make symmetric around zero
    
    logger.info(f"Value function range: [{c_min_raw:.4f}, {c_max_raw:.4f}] -> symmetric: [{c_min:.4f}, {c_max:.4f}]")
    ...
    # Initialize scatter plots with diverging colormap for better pos/neg distinction
    scatter_main = ax_main.scatter([], [], [], s=8, alpha=0.8, c=[], cmap=create_dual_colormap(), edgecolors='white', linewidth=0.1, vmin=c_min, vmax=c_max)
    
    # Plot ground truth with same color scheme and scale
    x_gt = gt_points[:, 0] 
    y_gt = gt_points[:, 1] 
    theta_gt = gt_points[:, 2]
    scatter_gt = ax_gt.scatter(x_gt, y_gt, theta_gt, s=4, alpha=0.8, c=c_gt, cmap=create_dual_colormap(), edgecolors='white', linewidth=0.1, vmin=c_min, vmax=c_max)
    ...
    cbar_main = plt.colorbar(scatter_main, ax=ax_main, shrink=0.6, aspect=10)
    cbar_main.set_label(f'Value Function [{c_min:.3f}, {c_max:.3f}]', fontsize=10, fontweight='bold')
    cbar_main.ax.tick_params(labelsize=8)
    
    cbar_gt = plt.colorbar(scatter_gt, ax=ax_gt, shrink=0.6, aspect=10)
    cbar_gt.set_label(f'Value Function [{c_min:.3f}, {c_max:.3f}]', fontsize=8, fontweight='bold')
